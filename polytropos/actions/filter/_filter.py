...
class Filter(Step):  # type: ignore # https://github.com/python/mypy/issues/5374
    ...
    @classmethod
    def build(cls, context: Context, schema: Schema, name: str, mappings: Optional[Dict] = None):  # type: ignore
        if mappings is None:
            mappings = {}
        logging.info('Building instance of filter class "%s"' % name)
        filters = load(cls)
        return filters[name](context=context, schema=schema, **mappings)
