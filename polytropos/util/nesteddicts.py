from typing import Optional, Any, Iterable, Dict, Sized, Iterator
...
def _do_get(target: Any, nodes: Sized) -> Optional[Any]:
    if len(nodes) == 0:
        return target
...
def get(target: Dict, spec: Sized, default: Any=NO_DEFAULT, accept_none: bool=True) -> Any:
...
def _do_put(target: Dict, spec_arr: Sized, value: Any) -> None:
...
def put(target: Dict, spec: Sized, value: Any) -> None:
...
def delete(target: Dict, spec: Sized) -> None:
...
def pop(target: Dict, spec: Sized, default: Any=NO_DEFAULT, accept_none: bool=True) -> Optional[Any]:
