...
class AssignAnnualBMI(Change):
    annual_weight_var: Decimal = VariableValidator(data_type=Decimal, temporal=1)
    height_var: Primitive = VariableValidator(data_type=[Decimal, Integer], temporal=-1)
    annual_bmi_var: Decimal = VariableValidator(data_type=Decimal, temporal=1)
