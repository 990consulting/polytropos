...
from polytropos.actions.filter.exists import DoesNotExist
from polytropos.ontology.variable import VariableId

@pytest.fixture()
def do_test(schema, composites, context) -> Callable:
    def _do_test(var_id: VariableId, expected: List[Composite]):
        the_filter: Filter = DoesNotExist(context, schema, var_id, narrows=False)
...
def test_primitive(composites, do_test):
    var_id: str = "i_Text"
    expected = [composites[0]]  # Empty string is considered to "exist"
    do_test(var_id, expected)

def test_folder(composites, do_test):
    var_id: str = "i_Folder"
    expected = composites[:-1]
    do_test(var_id, expected)
