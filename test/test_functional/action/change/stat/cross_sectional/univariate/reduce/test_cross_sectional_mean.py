...
from polytropos.actions.changes.stat.cross_sectional.reduce import CrossSectionalMean
...
def test_list_mean(schema, i_composite):
    change: Change = CrossSectionalMean(schema, {}, list_in_root, target_decimal, argument=int_in_list)
    change(i_composite)

    expected: Dict = copy.deepcopy(i_composite.content)
    expected["immutable"]["targets"] = {
        "target_decimal": 0.0    # (75 + 0 + (-75)) / 3 == 0
    }
    assert i_composite.content == expected

def test_keyed_list_mean(schema, i_composite):
    change: Change = CrossSectionalMean(schema, {}, kl_in_root, target_decimal, argument=decimal_in_kl)
    expected: Dict = copy.deepcopy(i_composite.content)
    expected["immutable"]["targets"] = {
        "target_decimal": 77 / 3
    }
