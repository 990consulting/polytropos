"""Regression test for bug whereby SourceCoverage failed to detect some variables and sources because they
...
def module_source_track(half_spec) -> Callable:
    def _track(prefix, track_name) -> Track:
        first_half: Dict = half_spec("source", 1, "first_missing", prefix, track_name)
        second_half: Dict = half_spec("source", 2, "second_missing", prefix, track_name)
...
def module_target_track(half_spec) -> Callable:
    def _track(prefix, track_name) -> Track:
        first_half: Dict = half_spec("target", 1, "first_missing", prefix, track_name)
        second_half: Dict = half_spec("target", 2, "second_missing", prefix, track_name)
        spec: Dict = {**first_half, **second_half}
        return Track.build(spec, None, track_name)
    return _track
...
def output_basepath() -> str:
    ...
    return "/tmp/source_cvg_missing_%s" % dirname
