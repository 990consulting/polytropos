...
@pytest.fixture(scope="module")
def module_target_spec(half_spec) -> Callable:
    def _spec(prefix, track_name) -> Dict:
...
@pytest.fixture(scope="module")
def module_target_schema(module_source_schema, module_target_spec) -> Schema:
    temporal_spec: Dict = module_target_spec("t", "temporal")
    temporal: Track = Track.build(temporal_spec, module_source_schema.temporal, "temporal")

    immutable_spec: Dict = module_target_spec("i", "immutable")
    immutable: Track = Track.build(immutable_spec, module_source_schema.immutable, "immutable")

    target_schema = Schema(temporal, immutable, source=module_source_schema)
    return target_schema


@pytest.fixture(scope="module")
def module_target_spec_with_transient(half_spec) -> Callable:
    def _spec(prefix, track_name) -> Dict:
        first_half: Dict = half_spec("target", 1, "first", prefix, track_name)
        second_half: Dict = half_spec("target", 2, "second", prefix, track_name)
        spec: Dict = {**first_half, **second_half}
...
@pytest.fixture(scope="module")
def module_target_schema_with_transient(module_source_schema, module_target_spec_with_transient) -> Schema:
    temporal_spec: Dict = module_target_spec_with_transient("t", "temporal")
    temporal: Track = Track.build(temporal_spec, module_source_schema.temporal, "temporal")
    immutable_spec: Dict = module_target_spec_with_transient("i", "immutable")
    immutable: Track = Track.build(immutable_spec, module_source_schema.immutable, "immutable")
    target_schema = Schema(temporal, immutable, source=module_source_schema)
    return target_schema
...
def output_basepath() -> str:
    ...
    return "/tmp/source_cvg_%s" % dirname
