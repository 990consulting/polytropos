...
@pytest.mark.parametrize("abs_path, var_id", [
    (["foo"], "t_folder_1"),
    (["foo", "a_list", "a_named_list", "some_text"], "t_named_list_text_1"),
    (["first_immutable_folder", "a_list", "a_named_list", "some_text"], "i_named_list_text_1"),
    (["second_immutable_folder", "a_list", "a_named_list", "some_text"], "i_named_list_text_2")
])
def test_lookup_after_rename(schema, abs_path: ListType[str], var_id: str):
    root_to_rename: Variable = schema.get("t_folder_1")
    root_to_rename.name = "foo"
    expected: Variable = schema.get(var_id)
    assert expected is not None
    actual: Variable = schema.lookup(abs_path)
    assert actual is expected
...
@pytest.mark.parametrize("abs_path, var_id", [
    ...
    (["first_immutable_folder", "a_list", "a_named_list"], "i_named_list_1"),
    (["first_immutable_folder", "a_list", "a_named_list", "some_text"], "i_named_list_text_1"),
    (["first_immutable_folder", "another_list"], "i_list_2"),
    (["first_immutable_folder", "another_list", "some_text"], "i_list_text_2"),
    (["first_immutable_folder", "another_list", "a_named_list"], "i_named_list_2"),
    (["first_immutable_folder", "another_list", "a_named_list", "some_text"], "i_named_list_text_2"),
    (["second_immutable_folder"], "i_folder_2"),
    (["second_immutable_folder", "some_text"], "i_folder_text_2")
])
def test_lookup_after_move(schema, abs_path: ListType[str], var_id: str):
    schema.immutable["i_list_2"].name = "another_list"
    schema.immutable.move("i_list_2", "i_folder_1", 0)
    expected: Variable = schema.get(var_id)
    assert expected is not None
    actual: Variable = schema.lookup(abs_path)
    assert actual is expected
